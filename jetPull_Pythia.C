...
void jetPull_Pythia(Int_t startfile = 0,
  ...
  for(int ifile = startfile; ifile < endfile; ifile++){
	...
	for(Long64_t npart = 0; npart < ngen; ++npart){
	  ...
	  //Delta R^2 cut
	  if((Double_t)(TMath::Power(phi_v[njet] - genphi[npart],2) + TMath::Power(eta_v[njet] - geneta[npart],2)) > 0.3){
	    continue;
	  }

	  TVector2 partAxis = TVector2(geneta[npart], genphi[npart]);
	  TVector2 jetAxis = TVector2(eta_v[njet], phi_v[njet]);
	  TVector2 newJetPull; 
	  r[njet].Set((partAxis.X() - jetAxis.X()),(partAxis.Y() - jetAxis.Y()));
	  
	  cout << "NEW PART: " << genpt[npart] << " ================ " <<  TMath::Sqrt(r[njet].X()*r[njet].X() + r[njet].Y()*r[njet].Y()) << endl;
	  if(debug) cout << "y: " << geneta[npart] << endl;
	  if(debug) cout << "azimuthal: " << genphi[npart] << endl;
	  
	  jetPullMag = TMath::Sqrt(r[njet].X()*r[njet].X() + r[njet].Y()*r[njet].Y()) * genpt[ngen]/pt_v[njet];
	  newJetPull = jetPullMag*r[njet];
	...
	if(jetPullMag != 0 && njet == leadJet){
	  h_jetPull->Fill(jetPullMag, pThat_weight);
	  h_jetPull_2d->Fill(jetPull.X(), jetPull.Y());
	  h_jetPull_3d->Fill(jetPull.X(), jetPull.Y(), pt_v[njet]);
	  h_leadingJetpT->Fill(pt_v[njet], pThat_weight);
	  h_leadingJetphi->Fill(phi_v[njet], pThat_weight);
	}

	//calculate the pull angle of each subjet with the leading jet
	//PULL ANGLE LOOP =====================
	//for(Long64_t npull = 0; npull < NJets_Sel; ++npull){
	if(njet == leadJet) { continue; }
	if(jetPullMag == 0) { continue; }
	
	// the vector defining the difference between the lead jet and this particular jet
	TVector2 rpull; 
	rpull.Set(eta_v[leadJet]-eta_v[njet], phi_v[leadJet]-phi_v[njet]);
	...
	//fill in -- right now, this is making pt chunks by the weight of the nonleading jet
	h_pullAngle->Fill(pullAngle, pThat_weight);
	cout << "pt_leadJet " << pt_v[leadJet] << " pt_mainJet " << pt_v[njet] << " Pull Angle: " << pullAngle << "\n arccos inside " << (jetPull.X()*rpull.X() + jetPull.Y()*rpull.Y()) << " / " << (TMath::Sqrt(jetPull.X()*jetPull.X() + jetPull.Y()*jetPull.Y()) * TMath::Sqrt(rpull.X()*rpull.X() + rpull.Y()*rpull.Y())) <<  endl;
	if(pt_v[njet] < 30) { h_30->Fill(pullAngle, pThat_weight); }
	if(pt_v[njet] > 30 && pt_v[njet] < 60) { h_60->Fill(pullAngle, pThat_weight);  }
	if(pt_v[njet] > 60 && pt_v[njet] < 90) { h_90->Fill(pullAngle, pThat_weight); }
	if(pt_v[njet] > 90 && pt_v[njet] < 120) { h_120->Fill(pullAngle, pThat_weight); }
	//}//end pull angle loop =========

	//fill eta difference
	h_leadingJeteta->Fill(eta_v[leadJet] - eta_v[njet]);
    }
