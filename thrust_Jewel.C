...
void thrust_Jewel(Int_t startfile = 0,
  ...
  */
  ...
  for(int ifile = startfile; ifile < endfile; ifile++){
    ...
    //for(Long64_t nentry = 58796; nentry<58797; ++nentry){
	...
	for(Long64_t njet = 0; njet < NJets_Sel; ++njet){
	  ...
	  //fill jet variables
	  if(weighted) {
	    h_eta->Fill(eta_v[njet], pThat_weight);
	    h_phi->Fill(phi_v[njet], pThat_weight);
	    h_TBadpT->Fill(pt_v[njet], pThat_weight);
	  }
	  else {
	    h_TBadpT->Fill(pt_v[njet]);
	    h_eta->Fill(eta_v[njet]);
	    h_phi->Fill(phi_v[njet]);
	  }
